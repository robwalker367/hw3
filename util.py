...
class NANDProgram(object):
    ...
    def __init__(self, num_inputs, num_outputs, debug = False):
        ...
        self._allocate_shmurp_workspace_var = self.make_allocator('SHMURP')
...
#       Feel free to make any other allocators that you feel necessary.
    ...
    # def ADD_BIT_TO_LIST(self, val, lst):
    #   carried_value = self._allocate_shmurp_workspace_var()
    #   carried_value = val
    #   output = []
    #   for i in range(len(lst)):

    #     temp = self._allocate_shmurp_workspace_var()
    #     new_carried = self._allocate_shmurp_workspace_var()
    #     self.ADD_3(temp, new_carried, lst[i], carried_value, self.ZERO("ZERO"))
    #     output.append(temp)
    #     carried_value = new_carried

    #   output.append(carried_value)
    #   return output
    ...
    def ADDER(self, acc_lst, row_string, N):
        ...
        for i in range(1, N - 1):
            ...
            carry = self.allocate()
            output_var = self.allocate()

            self.ADD_3(output_var, carry,
                    row_string[i], acc_lst[i], last_carry)
            new_output.append(output_var)
